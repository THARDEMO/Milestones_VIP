...
function listingInstance( parentID, {id, title, rank, favourite}) {
    ...
    const favouriteButton = document.createElementNS( 'http://www.w3.org/2000/svg', 'svg');
          favouriteButton.classList.add( 'favouriteStar');
          favouriteButton.setAttribute( 'height', 21)
          favouriteButton.setAttribute( 'width', 23)

    //IF FAV SET GOLDEN COLOR else SET GRAY COLOR      
    const starFill = favourite ? '#ddcf08' : '#00000045';
    //CREATE STAR
    ...
    path.onclick = (e) => patchFavourite( parentID, id ) 

    favouriteButton.append( path);
    instanceContainer.append( favouriteButton)
