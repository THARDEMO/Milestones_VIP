...
function listingInstance( parentID, instanceData) {
    ...
    const favContainer = createStar( favourite );
    ...
    favContainer.onclick = () => {
        ...
        // console.log( bool, favourite)
    
        STATE.Patch( { entity: parentID, ID: id, field: 'favourite', value: bool} ); 
    };
    deleteContainer.onclick = () => STATE.Delete( { entity: parentID, ID: id });
...
function patch_instance_listing( entity, instanceData ) {
    ...
    const starFill = instanceData.favourite ? '#daa520' : '#00000045';
    listingInstance.querySelector( '.favouriteStar').dataset.favourite = instanceData.favourite;
